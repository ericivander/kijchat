...
public class DES {
    ...
    public static void main(String[] a) {
        try {
            ...
            byte[] theExp = null;
            byte[] IV = null;
            IV = hexToBytes("a11cfd40493dd034");
            System.out.println(bytesToHex(IV));
            theKey = "hehehehe".getBytes();
            theMsg = "penipeni".getBytes();
            System.out.println(theMsg.length);
            byte[] baru = new byte[(int)Math.ceil((double)theMsg.length / 8.0) * 8];
            Arrays.fill(baru, (byte) 0);
            System.arraycopy(theMsg, 0, baru, 0, theMsg.length);
            System.out.println(bytesToHex(baru));
            
            byte[][] subKeys = getSubkeys(theKey);
            byte plain[] = new byte[8];
            byte[] theCph = null;
            for (int i = 0; i < baru.length; i += 8){
                System.arraycopy(baru, i, plain, 0, 8);
                if (i == 0)
                    theCph = cipher(IV, subKeys, "encrypt");
                else
                    theCph = cipher(theCph, subKeys, "encrypt");
                byte[] ctext = doXORBytes(plain, theCph);
            ...
            System.out.println("Message : " + bytesToHex(baru));
